function AOptimal =solveBolaObjQuadKernel(A0, kGramMatrix, labels)
...
epsilon =1e-4;
Zkik = computeZvalues(A0, kGramMatrix, labels, isigma,'quad', epsilon);
...
newCost=modBolaQuadObjKernelLegendreNew(A0, kGramMatrix, Zkik, labels, isigma, Lambda);
f(1) = newCost;
if(VERBOSE)
    disp(['Iter ' num2str(0) ':']);
...
while(1)
    ...
    Zkik = computeZvalues(AOld, kGramMatrix, labels, isigma,'quad', epsilon);
    Yker = computeYker(Zkik, kGramMatrix, labels, isigma);
    [Uker, Sker, Vker, Lambda] = computeA(Yker, gHalfInv, labels);  

    %Evaluation the objective function
    prevCost = newCost;
    newCost  = modBolaQuadObjKernelLegendreNew(AOld, kGramMatrix, Zkik, labels, isigma, Lambda);
    f(nn+1)  = newCost;    
    ...
    if(costDiff < convgThresh)
        if(VERBOSE); 
            disp('Minimum change in cost reached --TRUE');
        end
